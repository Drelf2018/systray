...
func ToICON(data []byte) ([]byte, error) {
	...
	if lazy, ok := img.(*svg.Image); ok {
		...
		// sizes, single argument scale()).
		//
		// An SVG is rasterized straight at the size the platform asks for
		// (iconSize) rather than at its intrinsic size and then scaled down: the
		// vector is drawn at the size it is wanted at, so there is no intermediate
		...
		// blobs. The successor scales the stroke by sqrt(|det|) of the transform,
		// so drawing at the asked-for size comes out right. Do not move back to
